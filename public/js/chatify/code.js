...
function IDinfo(id, type) {
  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        // avatar photo
        var avatar = data.user_avatar;
        avatar = avatar.replace('http://localhost/', url_avatarUser);
        $(".messenger-infoView")
          .find(".avatar")
...
function getSharedPhotos(user_id) {
  $.ajax({
    ...
    success: (data) => {
        ...
        shared = shared.replaceAll('http://localhost/', url_avatarUser);
        data.shared
        $(".shared-photos-list").html(shared);
    },
    error: () => {
      console.error("Server error, check your response");
    },
